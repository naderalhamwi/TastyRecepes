...
public class RecipeBean {
    ...
    public int createRecipe(Recipe recipe){
        try (Connection con1 = ConnectionFactory.getConnection()){
           String insertRecipt = String.format("INSERT INTO recept (title, userName) VALUES ('%s', '%s');", recipe.getTitle(), recipe.getUserName());
           ...
           try (Connection con5 = ConnectionFactory.getConnection()){
               String insertlikes = String.format("INSERT INTO likesamount VALUES ((SELECT max(receptID) FROM recept),'%d');",0);
                
                PreparedStatement stmt5 = con5.prepareStatement(insertlikes );
                stmt5.executeUpdate();
                
            } catch (Exception e) {
                System.out.println("Error RecepiBean.CreateRecipe.con5 " +e.getMessage());
            }
   ...
   public int likeRecepi(int recipeId, int likes ){
       try (Connection con = ConnectionFactory.getConnection()){
           String deleteRecipe = String.format("UPDATE likesamount SET likes='%d' WHERE receptId='%d'", likes, recipeId);

            PreparedStatement pstmt1 = con.prepareStatement(deleteRecipe);
            int rows = pstmt1.executeUpdate();

            return rows;
       } catch (Exception e) {
           System.out.println("RecipeBean.likeRecepi" + e.getMessage());
            return 0;
       }
   }
   
   public int commentOnRecipe(Comments comments){
       try (Connection con = ConnectionFactory.getConnection()){
           String commentOnRecipe = "INSERT INTO comment (content, date, receptId, userName) VALUES (?,?,?,?)";

            PreparedStatement pstmt = con.prepareStatement(commentOnRecipe);
            pstmt.setString(1, comments.getContent());
            pstmt.setString(2, comments.getDate());
            pstmt.setInt(3, comments.getReceptId());
            pstmt.setString(4, comments.getUserName());
            int rows = pstmt.executeUpdate();

            return rows;
       } catch (Exception e) {
           System.out.println("RecipeBean.commentOnRecipe" + e.getMessage());
            return 0;
       }
   }
   public int commentOncomment(Comments comments){
       try (Connection con = ConnectionFactory.getConnection()){
           String commentOnRecipe = "INSERT INTO replaydcomments (relatedUserName, userName, parentId, date, content) VALUES (?,?,?,?,?)";

            PreparedStatement pstmt = con.prepareStatement(commentOnRecipe);
            pstmt.setString(1, comments.getRelatedUserName());
            pstmt.setString(2, comments.getUserName());
            pstmt.setInt(3, comments.getParentId());
            pstmt.setString(4, comments.getDate());
            pstmt.setString(5, comments.getContent());
            int rows = pstmt.executeUpdate();

            return rows;
       } catch (Exception e) {
           System.out.println("RecipeBean.commentOncomment" + e.getMessage());
            return 0;
       }
   }
}